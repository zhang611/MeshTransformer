...
class SelfAttention(nn.Module):
    def __init__(self, embed_size, heads):
        ...
        self.values = nn.Linear(self.head_dim, self.head_dim, bias=False)    # 64*64
        self.keys = nn.Linear(self.head_dim, self.head_dim, bias=False)
        self.queries = nn.Linear(self.head_dim, self.head_dim, bias=False)  # 都是64*64的全连接,就是矩阵
        self.fc_out = nn.Linear(heads * self.head_dim, embed_size)          # 输出是512*512

    def forward(self, values, keys, query, mask):
        N = query.shape[0]  # 训练样本的数量，batch数    4
...
class Transformer(nn.Module):
    ...
    def make_src_mask(self, src):
        ...
        src = src.sum(dim=-1)
        src_mask = (src != self.src_pad_idx).unsqueeze(1).unsqueeze(2)  # (2, 9)->(2, 1, 1, 9)  (4, 1, 1, 300)
        # (N, 1, 1, src_len)
